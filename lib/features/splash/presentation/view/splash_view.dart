...
class _SplashViewState extends State<SplashView> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // 4 Seconds matches your Bullet sound duration
    _controller = AnimationController(
      duration: const Duration(seconds: 4),
      vsync: this,
    );

    // ✅ CHANGED: -1.1 to 1.1 makes the bike move SLOWER
    // Now it spends ~3.6 seconds visible on screen instead of ~2 seconds.
    _offsetAnimation = Tween<Offset>(
      begin: const Offset(-1.1, 0.0), 
      end: const Offset(1.1, 0.0),
    ).animate(CurvedAnimation(
      parent: _controller,
      curve: Curves.linear, // Linear feels more like a steady Bullet "Dug-Dug"
    ));
  ...
  Future<void> _initializeAndStart() async {
    try {
      // Pre-load the sound
      await _audioPlayer.setSource(AssetSource('sounds/bike_sound.mp3'));
      await _audioPlayer.setVolume(1);

      // Give Lottie 200ms to be ready
                  ...
                  SlideTransition(
                    ...
                    child: Padding(
                      ...
                      child: SizedBox(
                        height: 200, 
                        width: 300, 
                        child: Lottie.asset('assets/animations/motorcycle.json', fit: BoxFit.contain)
                      ),
                    ),
                  ),
                ],
